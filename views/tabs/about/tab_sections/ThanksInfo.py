...
class ThanksInfo(TkViewElements.TkLabelFrame):
    def __init__(self, about: 'About.About', label_text: str = "Special Thanks to the following Supporters", column: int = 0, row: int = 2, sticky: tk.constants = tk.EW):
        ...
        self.about = about
        self.supporters = ["daisame.bsky.social", "CBesty"]  # todo: read supporters in from text file in root dir (next to version.txt)
        self.supporters = list(self.organize_supporters(5))
    ...
    def organize_supporters(self, n):
        """
