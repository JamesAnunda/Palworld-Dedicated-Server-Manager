...
class ApplicationInfo(TkViewElements.TkLabelFrame):
    def __init__(self, about: 'About.About', label_text: str = "Application Info", column: int = 0, row: int = 0, sticky: tk.constants = tk.N):
        ...
        row = 0
        ttk.Label(self, text="Application Version: ").grid(column=0, row=row, padx=10, pady=10, sticky=tk.N)
        with open(os.path.join(self.about.application.root_path, 'version.txt'), 'r') as file:
            version = file.read().replace('\n', '').strip()
        ttk.Label(self, text=version).grid(column=1, row=row, padx=10, pady=10)

        row += 1
        ttk.Button(self, text="Check for Updates", command=self.check_for_updates).grid(column=0, row=row, columnspan=2, padx=10, pady=10)

        row += 1
        ttk.Button(self, text="Report a Bug", command=Utilities.report_bug).grid(column=0, row=row, columnspan=2, padx=10, pady=10)
